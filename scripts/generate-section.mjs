...
const [, , rawFolder, rawName] = process.argv;

if (!rawFolder || !rawName) {
  console.error("Uso: npm run section subcarpeta Nombre");
  process.exit(1);
}
...
const folderPath = toKebabPath(rawFolder);
const importFolderPath = folderPath.split(path.sep).join("/");
const baseName = toPascalCase(rawName);

if (!folderPath) {
  console.error("La subcarpeta debe contener letras o numeros validos.");
  process.exit(1);
}
...
const sectionsDir = path.join(root, "app", "sections", folderPath);
const scssSectionsDir = path.join(root, "app", "sass", "sections", folderPath);
...
const exportLine = `export * from "./${importFolderPath}/${sectionName}";`;
...
const useLine = `@use "./sections/${importFolderPath}/${sectionId}";`;
